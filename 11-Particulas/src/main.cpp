...
GLuint textureTerrainBackgroundID, textureTerrainRID, textureTerrainGID, textureTerrainBID, textureTerrainBlendMapID, textureParticulaAguaID, textureParticleFountainID;
GLuint textureParticulaAgua;
...
std::map<std::string, glm::vec3> blendingUnsorted = {
		...
		{"particulasAgua", glm::vec3(0.0, 0.0, 0.0)}
};
	...
	while (psi) {
		...
		for(std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++){
			if(it->second.first.compare("aircraft") == 0){
				...
				modelMatrixParticulasFuente[3][1] += 10.0;
