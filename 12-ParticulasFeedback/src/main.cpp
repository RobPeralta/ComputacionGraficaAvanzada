...
GLuint nParticlesFire = 500;
GLuint posBuf[2], velBuf[2], age[2];
GLuint particleArray[2];
GLuint feedback[2];
GLuint drawBuf = 1;
float particleSize = 2.5, particleLifetime = 10.0;
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	shaderParticlesHumo.setVectorFloat3("Accel", glm::value_ptr(glm::vec3(0.0f, 0.05f, 0.0f)));
	shaderParticlesHumo.setVectorFloat3("Emitter", glm::value_ptr(glm::vec3(0.0f, 10.0f, 0.0f)));
...
void applicationLoop() {
	...
	while (psi) {
		...
		shaderParticlesFire.setVectorFloat3("colorFuego", glm::value_ptr(glm::vec3(0.0, 0.0, 1.0)));
		// Settea la matriz de vista y projection al shader para el fuego
		...
		shaderParticlesHumo.setVectorFloat3("colorFuego", glm::value_ptr(glm::vec3(0.0, 0.0, 1.0)));
		...
		for(std::map<float, std::pair<std::string, glm::vec3> >::reverse_iterator it = blendingSorted.rbegin(); it != blendingSorted.rend(); it++){
			...
			else if (it->second.first.compare("humo") == 0) {
				/**********
				 * Init Render particles systems
				 */
				 lastTimeParticlesAnimationFire = currTimeParticlesAnimationFire;
